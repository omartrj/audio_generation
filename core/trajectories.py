...
class RandomWalkTrajectory(Trajectory):
    def __init__(self, room_config: dict, min_speed_kmh: float, max_speed_kmh: float, person_height: float = 1.5):
        self.room_config = room_config
        self.min_speed_kmh = min_speed_kmh
        self.max_speed_kmh = max_speed_kmh
    ...
    def generate(self, duration: float, dt: float) -> np.ndarray:
        ...
        for i in range(1, num_steps + 1):
            ...
            x = np.clip(x + step_size * np.cos(heading), -half_w, half_w)
            y = np.clip(y + step_size * np.sin(heading), -half_l, half_l)
